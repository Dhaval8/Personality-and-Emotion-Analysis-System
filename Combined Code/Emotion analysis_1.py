...
class PersonalityAnalyzer:
    ...
    def generate_summary(self):
        try:
            ...
            for trait in self.video_analyzer.traits:
                values = self.video_analyzer.trait_history[trait]
                if values:
                    plt.plot(values, label=trait.capitalize(),
                             color=[x/255 for x in self.video_analyzer.trait_colors[trait]])
            plt.title('Personality Trait Analysis Trends (Video Analysis)')
            plt.xlabel('Frames')
            plt.ylabel('Score (%)')
            plt.legend()
            plt.grid(True)
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            plt.savefig(f'personality_analysis_video_{timestamp}.png')
            plt.close()
            ...
            plt.savefig(f'personality_analysis_histogram_video_{timestamp}.png')
            plt.close()
