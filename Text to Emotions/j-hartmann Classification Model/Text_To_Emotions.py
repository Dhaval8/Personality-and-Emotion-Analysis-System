...
def analyze_emotions_with_Jhartmann_Classification_Model(text, chunk_size=450):
    # Initialize emotion analyzer with an emotion-specific model
    ...
    for chunk in chunks:
        ...
        print(f"Result: {result}")  # Debug the structure
        
        if isinstance(result, list) and all(isinstance(res, dict) for res in result[0]):
            # Process if result is a list of dictionaries
    ...
    return {emotion: np.mean(scores) for emotion, scores in final_scores.items()}
...
def plot_emotions(emotion_scores):
    ...
    plt.xlabel('Emotions')
    plt.ylabel('Scores')
    plt.title('Average Emotion Analysis using j-hartmann Classification Model')
...
Then coming to phase two, the communication skills
...
"""
emotion_scores = analyze_emotions_with_Jhartmann_Classification_Model(text)
