...
def analyze_emotions_bert(text, chunk_size=450):
    """
    Analyze emotions using a public emotion classification model.
    """
...
def plot_emotions(emotion_scores):
    """
    Plot all emotions detected by the model.
    """
    emotions = list(emotion_scores.keys())
    scores = list(emotion_scores.values())
    
    plt.figure(figsize=(10, 5))
    bars = plt.bar(emotions, scores, color='lightseagreen')
    plt.xlabel('Emotions')
    plt.ylabel('Scores')
...
def analyze_and_compare_with_threshold(text, threshold=0.1):
    """
    Analyze emotions and show only those above a certain threshold.
    
    Args:
        text (str): Input text to analyze
        threshold (float): Minimum score threshold to include emotion
    """
    emotion_scores = analyze_emotions_bert(text)
    
    # Filter emotions above threshold
    significant_emotions = {k: v for k, v in emotion_scores.items() if v >= threshold}
    
    print(f"\nSignificant emotions (score >= {threshold}):")
...
if __name__ == "__main__":
    ...
    # Show significant emotions with threshold
    significant_emotions = analyze_and_compare_with_threshold(text, threshold=0.1)
